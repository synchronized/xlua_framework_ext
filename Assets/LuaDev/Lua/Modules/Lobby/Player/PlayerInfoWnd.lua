...
function PlayerInfoWnd:Awake()
    ...
    self.btnMask = self.transform:Find("UIWindow/bg"):GetComponent(typeof(UnityEngine.UI.Button))
    self.btnMask.onClick:AddListener(function ()
        self:CloseUI()
    end)

    local btnBack = self.transform:Find("UIWindow/OverLayer/btnBack"):GetComponent(typeof(UnityEngine.UI.Button))
